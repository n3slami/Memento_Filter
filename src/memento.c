...
static inline uint64_t get_kth_word_from_slot(const QF *qf, uint64_t index, uint64_t k)     // NEW IN MEMENTO
{
	/* Should use __uint128_t to support up to 64-bit remainders, but gcc seems
	 * to generate buggy code.  :/  */
	uint64_t res;
                                            ...
                                            const uint64_t pos, uint64_t new_memento)   // NEW IN MEMENTO
{
    ...
    if (size_two_prefix_set) {
        if (qf->metadata->bits_per_slot < 2 * qf->metadata->memento_bits) {
            ...
            if (new_memento < m1) {
                ...
                APPEND_WRITE_PAYLOAD_WORD(qf, payload, current_full_prefix, value,
                                          qf->metadata->bits_per_slot, dest_bit_pos,
                                          dest_block_ind);
    ...
    uint64_t data_bit_pos = (ind % QF_SLOTS_PER_BLOCK) * qf->metadata->bits_per_slot;
    uint64_t data_block_ind = ind / QF_SLOTS_PER_BLOCK;
    GET_NEXT_DATA_WORD_IF_EMPTY(qf, data, filled_bits, memento_bits, data_bit_pos, data_block_ind);
    ...
    if (unary_count) {
        while (unary_count) {
            APPEND_WRITE_PAYLOAD_WORD(qf, payload, current_full_prefix, max_memento_value,
                                      memento_bits, dest_bit_pos, dest_block_ind);
            unary_count--;
        }
        for (cnt = memento_count; cnt; cnt /= max_memento_value) {
            const uint64_t appendee = cnt % max_memento_value;
            APPEND_WRITE_PAYLOAD_WORD(qf, payload, current_full_prefix, appendee,
                                      memento_bits, dest_bit_pos, dest_block_ind);
        }
    }
    else {
        APPEND_WRITE_PAYLOAD_WORD(qf, payload, current_full_prefix, memento_count,
                                  memento_bits, dest_bit_pos, dest_block_ind);
    }
    ...
    for (uint32_t i = 0; i < memento_count - 1; i++) {
        if (!written_new_memento && mementos[i] > new_memento) {
            ...
            APPEND_WRITE_PAYLOAD_WORD(qf, payload, current_full_prefix, new_memento,
                                      memento_bits, dest_bit_pos, dest_block_ind);
        }
        const uint64_t memento = mementos[i];
        APPEND_WRITE_PAYLOAD_WORD(qf, payload, current_full_prefix, memento,
                                  memento_bits, dest_bit_pos, dest_block_ind);
    }
        ...
        void *buffer, uint64_t buffer_len, const uint64_t orig_quotient_bit_cnt)    // NEW IN MEMENTO
{
    ...
    bits_per_slot++;    // For variable length fingerprints
...
int64_t qf_resize_malloc(QF *qf, uint64_t nslots)   // NEW IN MEMENTO
{
	...
	QF new_qf;
	if (!malloc_filter(&new_qf, nslots, qf->metadata->key_bits + 1,
...
int64_t qf_insert_single(QF *qf, uint64_t key, uint64_t memento, uint8_t flags)     // NEW IN MEMENTO 
{
    ...
    if (is_occupied(qf, hash_bucket_index)) {
        ...
        while (true) {
            ...
            if (highbit_position(current_fingerprint) == qf->metadata->fingerprint_bits) {
        ...
        if (add_to_sorted_list) {
            ...
            res = add_memento_to_sorted_list(qf, hash_bucket_index, insert_index, memento);
            ...
            for (uint32_t i = hash_bucket_index / QF_SLOTS_PER_BLOCK + 1; i <= next_empty_slot / QF_SLOTS_PER_BLOCK; i++) {
                if (get_block(qf, i)->offset + 1ULL <= BITMASK(8 * sizeof(qf->blocks[0].offset)))
                    get_block(qf, i)->offset++;
            }
            set_slot(qf, insert_index, (hash_fingerprint << qf->metadata->memento_bits) | memento);
            METADATA_WORD(qf, runends, runend_index) &= ~(1ULL << ((runend_index % QF_SLOTS_PER_BLOCK) % 64));
            METADATA_WORD(qf, runends, runend_index + 1) |= 1ULL << (((runend_index + 1) % QF_SLOTS_PER_BLOCK) % 64);
            modify_metadata(qf, &qf->metadata->ndistinct_elts, 1);
            modify_metadata(qf, &qf->metadata->noccupied_slots, 1);
            res = insert_index - hash_bucket_index;
        }
...
void qf_bulk_load(QF *qf, uint64_t *sorted_hashes, uint64_t n, uint8_t flags)   // NEW IN MEMENTO 
{
    ...
    for (uint64_t i = 1; i < n; i++) {
        ...
        if (new_prefix == prefix)
            ...
            const uint32_t slots_written = write_prefix_set(qf, current_pos, (1ULL << qf->metadata->fingerprint_bits) 
            ...
            if (current_run != next_run) {
                METADATA_WORD(qf, occupieds, current_run) |= (1ULL << ((current_run % QF_SLOTS_PER_BLOCK) % 64));
                METADATA_WORD(qf, runends, (current_pos - 1)) |= (1ULL << (((current_pos - 1) % QF_SLOTS_PER_BLOCK) % 64));
                for (uint64_t block_ind = current_run / QF_SLOTS_PER_BLOCK + 1; block_ind <= (current_pos - 1) / QF_SLOTS_PER_BLOCK; block_ind++) {
                    const uint64_t cnt = current_pos - (old_pos < block_ind * QF_SLOTS_PER_BLOCK ? block_ind * QF_SLOTS_PER_BLOCK : old_pos);
                    if (get_block(qf, block_ind)->offset + cnt < BITMASK(8 * sizeof(qf->blocks[0].offset)))
                        get_block(qf, block_ind)->offset += cnt;
                    else
                        get_block(qf, block_ind)->offset = (uint8_t) (BITMASK(8 * sizeof(qf->blocks[0].offset)));
                }
                current_run = next_run;
                old_pos = current_pos;
                current_pos = current_run < current_pos ? current_pos : current_run;
            }
        }
    }
    const uint32_t slots_written = write_prefix_set(qf, current_pos, (1ULL << qf->metadata->fingerprint_bits) 
...
{
#ifdef DEBUG
        fprintf(stderr, "REJUVINATING new_ind=%d other_mementos=[", new_ind);
        for (int i = 0; i < memento_cnt; i++) {
            fprintf(stderr, "%lu, ", mementos[i]);
        }
        fprintf(stderr, "\b\b]\n");
    ...
    // Remove the existing mementos
    bool inserting_new_memento = new_ind >= 0;
    uint32_t slots_emptied = 0;
    int64_t runstart_index = hash_bucket_index == 0 ? 
                                0 : run_end(qf, hash_bucket_index - 1) + 1;
    uint64_t insert_index = upper_bound_fingerprint_in_run(qf, runstart_index,
                                                            hash_fingerprint);
#ifdef DEBUG
        fprintf(stderr, "STARTING THE THING insert_index=%lu\n", insert_index);
#endif /* DEBUG */
    if (memento_cnt - inserting_new_memento) {
        bool handled[memento_cnt];
        memset(handled, false, memento_cnt);
        if (inserting_new_memento)
            handled[new_ind] = true;

        const uint32_t n_match = 20;

        int64_t runstart_index = hash_bucket_index == 0 ? 
                                    0 : run_end(qf, hash_bucket_index - 1) + 1;
        uint64_t fingerprint_pos = runstart_index;
        int64_t matching_position[n_match], match_amount[n_match], ind = 0;
...
#endif /* DEBUG */
            sorted_inds[--matching_cnt[match_amount[i]]] = i;
        }

        memset(match_amount, 0, sizeof(uint64_t) * n_match);
        int32_t processing_ind = ind - 1;
        int32_t mementos_left_to_handle = memento_cnt - inserting_new_memento;
        int32_t new_slot_count, old_slot_count;
#ifdef DEBUG
        fprintf(stderr, "START THE PURGE mementos_left_to_handle=%d\n", mementos_left_to_handle);
#endif /* DEBUG */
        while (mementos_left_to_handle && processing_ind >= 0) {
#ifdef DEBUG
            fprintf(stderr, "WTF? pos=%lu\n", matching_position[sorted_inds[processing_ind]]);
#endif /* DEBUG */
            mementos_left_to_handle -= remove_mementos_from_prefix_set(qf, 
                    matching_position[sorted_inds[processing_ind]], mementos,
                    handled, memento_cnt, &new_slot_count, &old_slot_count);
#ifdef DEBUG
            fprintf(stderr, "WELP mementos_left_to_handle=%d --- new_slot_count=%d vs. old_slot_count=%d\n", 
                            mementos_left_to_handle, new_slot_count, old_slot_count);
            qf_dump_block(qf, insert_index / QF_SLOTS_PER_BLOCK);
#endif /* DEBUG */
            match_amount[sorted_inds[processing_ind]] = (new_slot_count == -1 ?
                                            0 : old_slot_count - new_slot_count);
            processing_ind--;
        }
        
        for (uint32_t i = 0; i < ind; i++) {
            slots_emptied += match_amount[i];
            if (match_amount[i]) {
                if (i < ind - 1) {
#ifdef DEBUG
                    fprintf(stderr, "SHIFTING THE THINGY %u: match_amount=%lu\n", i, match_amount[i]);
                    qf_dump_block(qf, insert_index / QF_SLOTS_PER_BLOCK);
#endif /* DEBUG */
                    const uint64_t l = matching_position[i + 1];
                    const uint64_t r = insert_index - 1;
                    left_shift_slots_by_words(qf, l, r, -match_amount[i]);
#ifdef DEBUG
                    qf_dump_block(qf, insert_index / QF_SLOTS_PER_BLOCK);
#endif /* DEBUG */
                }
                insert_index -= match_amount[i];
            }
        }
    }
    int32_t bits_needed;
    if (memento_cnt <= 2) {
        bits_needed = memento_cnt * qf->metadata->bits_per_slot;
    }
    else {
        bits_needed = 2 * qf->metadata->bits_per_slot 
                    + (memento_cnt - 1) * qf->metadata->memento_bits;
    }
    uint32_t slots_needed = 0;
    while (bits_needed > 0) {
        slots_needed++;
        bits_needed -= qf->metadata->bits_per_slot;
    }

    if (slots_emptied < slots_needed) {
        make_n_empty_slots_for_memento_list(qf, hash_bucket_index, insert_index + slots_emptied,
                                slots_needed - slots_emptied);
    }
    else if (slots_emptied > slots_needed) {
        remove_replace_slots_and_shift_remainders_and_runends_and_offsets(qf, 
                0, hash_bucket_index, insert_index + slots_needed - 1, NULL, 0, 
                run_end(qf, hash_bucket_index) - runstart_index + 1);
    }
    modify_metadata(qf, &qf->metadata->noccupied_slots, slots_needed - slots_emptied);
#ifdef DEBUG
    fprintf(stderr, "WRITING THE NEW PREFIX SET insert_index=%lu\n", insert_index);
    qf_dump_block(qf, insert_index / QF_SLOTS_PER_BLOCK);
#endif /* DEBUG */
    write_prefix_set(qf, insert_index, hash_fingerprint, mementos,
                        memento_cnt);
#ifdef DEBUG
    perror("VS.");
    qf_dump_block(qf, insert_index / QF_SLOTS_PER_BLOCK);
    ...
    return insert_index - hash_bucket_index;
}
...
static int qfi_get(const QFi *qfi, uint64_t *key, uint64_t *mementos)   // NEW IN MEMENTO
{
    ...
    const uint32_t bucket_index_hash_size = qf->metadata->key_bits - qf->metadata->fingerprint_bits;
    const uint32_t orig_quotient_size = qf->metadata->original_quotient_bits;
	*key = (qfi->run >> (bucket_index_hash_size - orig_quotient_size))
                | (f1 << bucket_index_hash_size)
                | ((qfi->run & BITMASK(bucket_index_hash_size - orig_quotient_size)) << orig_quotient_size);
	return res;
}
	...
	if (qfi_end(qfi))
		...
		if (!is_runend(qfi->qf, qfi->current)) {
            ...
            if (GET_FINGERPRINT(qfi->qf, qfi->current) == 0) {
                fprintf(stderr, "JAHAN-DARA! II run=%lu current=%lu\n", qfi->run, qfi->current);
