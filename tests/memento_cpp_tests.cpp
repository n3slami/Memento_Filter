...
TEST_SUITE("standard memento") {
    ...
    TEST_CASE("updates") {
        ...
        SUBCASE("monte-carlo inserts and updates") {
            ...
            while (existing_keys.size() < n_elements) {
                ...
                while (rng() % geometric_modulo == 0) {
                    const uint64_t elem_memento = rng() & BITMASK(memento_bits);
                    REQUIRE_GE(memento.insert(elem_prefix, elem_memento, flags, 0), 0);
                    existing_keys.push_back((elem_prefix << memento_bits) | elem_memento);
                    check_set.insert({bucket_index, fingerprint, elem_memento, 0});
                }
            }
            ...
            for (uint32_t updates = 0; updates < n_updates; updates++) {
                ...
                if (elem_memento == new_elem_memento)
                    new_elem_memento ^= 1;
                REQUIRE_EQ(memento.update_single(elem_prefix, elem_memento, new_elem_memento, flags, 0), 0);
                ...
                REQUIRE(search_it != check_set.end());
                check_set.erase(search_it);
                check_set.insert({bucket_index, fingerprint, new_elem_memento, 0});
            }
            assert_memento_contents(check_set, memento);
        }

        SUBCASE("monte-carlo inserts and updates with payload") {
            const uint32_t payload_bits = 50;
            Memento<false> memento{n_slots, key_bits, memento_bits, Memento<false>::hashmode::Default, seed, 0, payload_bits};
            const uint32_t quotient_bits = memento.get_bucket_index_hash_size();
            const uint8_t flags = Memento<false>::flag_no_lock;
            ...
            for (uint32_t updates = 0; updates < n_updates; updates++) {
                ...
                if (elem_memento == new_elem_memento)
                    new_elem_memento ^= 1;
                REQUIRE_EQ(memento.update_single(elem_prefix, elem_memento, new_elem_memento, flags, new_elem_memento), 0);
                existing_keys[update_rank] = (elem & (~BITMASK(memento_bits))) | new_elem_memento;
                ...
                REQUIRE(search_it != check_set.end());
                check_set.erase(search_it);
                check_set.insert({bucket_index, fingerprint, new_elem_memento, new_elem_memento});
            }
            assert_memento_contents(check_set, memento);
        }
    }
    ...
    TEST_CASE("iterator") {
        ...
        SUBCASE("single partition") {
            ...
            auto it = memento.begin(100ULL, 0LL, 100ULL, BITMASK(memento_bits), Memento<false>::flag_no_lock);
        ...
        SUBCASE("single partition: skip first and last mementos") {
            ...
            auto it = memento.begin(100ULL, 3ULL, 100ULL, 29ULL, Memento<false>::flag_no_lock);
        ...
        SUBCASE("two partitions") {
            ...
            auto it = memento.begin(100ULL, 0ULL, 101ULL, BITMASK(memento_bits), Memento<false>::flag_no_lock);
        ...
        SUBCASE("five partitions with two gaps") {
            ...
            auto it = memento.begin(100ULL, 0ULL, 103ULL, BITMASK(memento_bits), Memento<false>::flag_no_lock);
        ...
        SUBCASE("insert") {
            ...
            for (int32_t i = 31; i > 0; i -= 3) {
                ...
                auto it = memento.begin(key_prefix, 0ULL, key_prefix, BITMASK(memento_bits), Memento<false>::flag_no_lock);
    ...
    TEST_CASE("iterator with payload") {
        ...
        SUBCASE("single partition") {
            ...
            const uint64_t l = (100ULL << memento_bits) | 0ULL;
            const uint64_t r = (100ULL << memento_bits) | BITMASK(memento_bits);
            auto it = memento.begin(100ULL, 0ULL, 100ULL, BITMASK(memento_bits), Memento<false>::flag_no_lock);
            for (int32_t i = 0; i < 14; i++) {
                REQUIRE_NE(it, memento.end());
                REQUIRE_EQ(*it, check_keys[i]);
                ++it;
    ...
    TEST_CASE("iterator") {
        ...
        for (uint32_t expansion_count = 0; expansion_count < expansions; expansion_count++) {
            ...
            for (auto it = prefix_sets.begin(); it != prefix_sets.end(); it++) {
                ...
                auto memento_it = memento.begin(key_prefix, 0, key_prefix, BITMASK(memento_bits), Memento<false>::flag_no_lock);
...
//        }
    ...
    TEST_CASE("updates") {
        ...
        for (uint32_t updates = 0; updates < n_updates; updates++) {
            ...
            for (uint32_t hash_length = max_hash_length; !found && hash_length > 0; hash_length--) {
                ...
                const uint64_t capped_hash = (((elem_prefix_hash & ~BITMASK(memento.get_original_quotient_bits())) | bucket_index) 
                                                            & BITMASK(hash_length)) | (1ULL << hash_length);
                auto search_it = check_set.find({capped_hash, elem_memento, 0});
                if (search_it != check_set.end()) {
                    check_set.erase(search_it);
                    check_set.insert({capped_hash, new_elem_memento, 0});
                    found = true;
                }
            }
            REQUIRE(found);
    ...
    TEST_CASE("updates with payload") {
        const uint32_t n_elements = 1000000;
        const uint32_t n_expansions = 4;
        const uint32_t n_updates = 500000;
        const uint32_t rng_seed = 2;
        ...
        for (uint32_t updates = 0; updates < n_updates; updates++) {
            ...
            for (uint32_t hash_length = max_hash_length; !found && hash_length > 0; hash_length--) {
                ...
                const uint64_t capped_hash = (((elem_prefix_hash & ~BITMASK(memento.get_original_quotient_bits())) | bucket_index)
                                                            & BITMASK(hash_length)) | (1ULL << hash_length);
                auto search_it = check_set.find({capped_hash, elem_memento, elem_memento});
                if (search_it != check_set.end()) {
                    check_set.erase(search_it);
                    check_set.insert({capped_hash, new_elem_memento, new_elem_memento});
                    found = true;
                }
            }
            REQUIRE(found);
