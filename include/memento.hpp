...
#define METADATA_WORD(field, slot_index)      \
  (get_block((slot_index) / slots_per_block_) \
       ->field[((slot_index) % slots_per_block_) / 64])
...
#define REMAINDER_WORD(i)                         \
  (reinterpret_cast<uint64_t *>(                  \
      &(get_block((i) / metadata_->bits_per_slot) \
            ->slots[8 * ((i) % metadata_->bits_per_slot)])))
 ...
 */
#define GET_NEXT_DATA_WORD_IF_EMPTY(data, filled_bits, bit_cnt, bit_pos, \
                                    block_ind)                           \
  {                                                                      \
...
#define GET_NEXT_DATA_WORD_IF_EMPTY_ITERATOR(filter, data, filled_bits,   \
                                             bit_cnt, bit_pos, block_ind) \
  {                                                                       \
    while (filled_bits < (bit_cnt)) {                                     \
      const uint64_t byte_pos = bit_pos / 8;                              \
      uint64_t *p = reinterpret_cast<uint64_t *>(                         \
          &filter.get_block((block_ind))->slots[byte_pos]);               \
      uint64_t tmp;                                                       \
      memcpy(&tmp, p, sizeof(tmp));                                       \
      tmp >>= (bit_pos % 8);                                              \
      data |= tmp << filled_bits;                                         \
      const uint64_t bits_per_block =                                     \
          slots_per_block_ * filter.metadata_->bits_per_slot;             \
      const uint64_t move_amount = 64 - filled_bits - (bit_pos % 8);      \
      filled_bits += move_amount;                                         \
      bit_pos += move_amount;                                             \
      if (bit_pos >= bits_per_block) {                                    \
 ...
 */
__attribute__((always_inline)) static inline uint32_t fast_reduce(uint32_t hash,
                                                                  uint32_t n) {
...
class Memento {
 ...
 public:
   ...
   */
  enum class hashmode { Default, Invertible, None };
  ...
  static constexpr uint32_t flag_key_is_hash =
      0x08;  // It is sometimes useful to insert a key that has already been
             // hashed.
 ...
 private:
  ...
  static constexpr uint32_t metadata_words_per_block_ =
      (slots_per_block_ + 63) / 64;
  ...
  struct __attribute__((__packed__)) qfblock {
    uint8_t offset;  // Also works with uint16_t, uint32_t, etc., but uint8_t
                     // seems just as fast
 ...
 public:
  ...
  explicit Memento(uint64_t nslots, uint64_t key_bits, uint64_t memento_bits,
                   hashmode hash_mode, uint32_t seed,
                   const uint64_t orig_quotient_bit_cnt = 0, uint64_t payload_bits = 0);
  ~Memento();
   ...
   * `false`.
   */
  void set_auto_resize(bool enabled) { metadata_->auto_resize = enabled; }
   ...
   */
  int64_t insert(uint64_t key, uint64_t memento, uint8_t flags, uint64_t payload = 0L);
   ...
   */
  int32_t range_query(uint64_t l_key, uint64_t l_memento, uint64_t r_key,
                      uint64_t r_memento, uint8_t flags) const;
  ...
  /* Hashing info */
  hashmode get_hashmode() const { return metadata_->hash_mode; }
  uint64_t get_hash_seed() const { return metadata_->seed; }
  __uint128_t get_hash_range() const { return metadata_->range; }

  /* Space usage info. */
  bool is_auto_resize_enabled() const { return metadata_->auto_resize; }
  uint64_t size_in_bytes() const { return metadata_->total_size_in_bytes; }
  uint64_t count_slots() const { return metadata_->nslots; }
  uint64_t count_occupied_slots() const { return metadata_->noccupied_slots; }

  /* Bit-sizes info. */
  uint64_t get_num_key_bits() const { return metadata_->key_bits; }
  uint64_t get_num_memento_bits() const { return metadata_->memento_bits; }
  uint64_t get_num_fingerprint_bits() const {
    return metadata_->fingerprint_bits;
  }
  uint64_t get_bits_per_slot() const { return metadata_->bits_per_slot; }
  ...
  class hash_iterator {
   ...
   public:
    hash_iterator(const Memento &filter) : filter_{filter} {};
   ...
   private:
    ...
    uint64_t cur_start_index_ =
        0;                    /**< The starting slot of the current cluster. */
    uint16_t cur_length_ = 0; /**< The length of the current cluster. */
    uint32_t num_clusters_ = 0; /**< The number of traversed clusters. */
    cluster_data *c_info_ =
        nullptr; /**< A list with information about the filter's clusters. */
  };
   ...
   *
   * @param position - The position in the filter to initialize the iterator
   * from.
   * @returns The corresponding iterator.
   */
 ...
 private:
  ...
  qfblock *get_block(uint64_t block_index) const {
    ...
    const uint32_t byte_offset =
        block_index * (sizeof(qfblock) - sizeof(qfblock::slots) +
  ...
  uint64_t get_slot_payload(uint64_t index) const;

  __attribute__((always_inline)) uint64_t get_fingerprint(
  ...
  void set_slot_payload(uint64_t index, uint64_t payload);
  ...
  bool probably_is_empty(uint64_t slot_index) const {
    return get_slot(slot_index) == 0 && !is_occupied(slot_index) &&
           !is_runend(slot_index);
  }

  uint64_t find_first_empty_slot(uint64_t from) const;

  uint64_t get_number_of_consecutive_empty_slots(uint64_t first_empty,
                                                 uint64_t goal_slots) const;

  void shift_remainders(const uint64_t start_index, const uint64_t empty_index);

  void find_next_n_empty_slots(uint64_t from, uint64_t n,
   ...
   */
  uint32_t find_next_empty_slot_runs_of_size_n(uint64_t from, uint64_t n,
                                               uint64_t *indices) const;
  ...
  /**
   * Shifts the filter's `runends` bitmap to the right starting from `first` to
   * `last`, inclusive, by `distance` bits. Bits shifter over will be
   * overwritten, while the empty bits resulting from the shift will be zeroed
   * out.
   *
  ...
  /**
   * Removes `old_length` slots starting at `overwrite_index` and replaces
   * them with the `total_remainders` values store in `remainders`. If
   * `operation` is set to `true`, the corresponding `occupieds` bits in the
   * filter will be set to zero if the run becomes empty.
   *
   * @param operation - If `true`, the `occupieds` of the run will be reset
   * if the run becomes empty.
   * @param bucket_index - The bucket whose run is being modified.
   * @param overwrite_index - The slot in the filter where overwrites will
   * happen from.
   * @param remainders - A list new remainders to move into the filter.
   * @param total_remainders - The length of the list of new remainders.
   * @param old_length - The number of slots being modified.
   */
  int32_t remove_replace_slots_and_shift_remainders_and_runends_and_offsets(
      bool operation, uint64_t bucket_index, uint64_t overwrite_index,
   ...
   * the filter has ran out of space.
   */
  int32_t make_n_empty_slots_for_memento_list(uint64_t bucket_index,
                                              uint64_t pos, uint32_t n);
   ...
   *
   * @param pos[in] - The position of the keepsake box, i.e., the slot where
   * it's fingerprint is stored in.
   ...
   * box.
   */
  int32_t remove_mementos_from_prefix_set(const uint64_t pos,
                                          const uint64_t *mementos,
                                          bool *handled,
                                          const uint32_t memento_cnt,
                                          int32_t *new_slot_count,
                                          int32_t *old_slot_count);
   ...
   */
  __attribute__((always_inline)) uint64_t number_of_slots_used_for_memento_list(
      uint64_t pos) const;
   ...
   */
  __attribute__((always_inline)) int64_t next_matching_fingerprint_in_run(
      uint64_t pos, const uint64_t fingerprint) const;
   ...
   * the end of the run.
   */
  uint64_t lower_bound_fingerprint_in_run(uint64_t pos,
   ...
   */
  uint64_t upper_bound_fingerprint_in_run(uint64_t pos,
                                          uint64_t fingerprint) const;
   ...
   * the keepsake box.
   */
  __attribute__((always_inline)) uint64_t lower_bound_mementos_for_fingerprint(
      uint64_t pos, uint64_t target_memento) const;
};

inline bool Memento::spin_lock(volatile int *lock, uint8_t flag) {
...
#ifdef LOG_WAIT_TIME
  spin_lock(&runtimedata->metadata_lock, runtimedata->num_locks,
            flag_wait_for_lock);
#else
...
// this function return only the fingerprint+memento content and not the payload
inline uint64_t Memento::get_slot(uint64_t index) const {
  ...
  uint64_t size_to_get = metadata_->fingerprint_bits + metadata_->memento_bits;
  uint64_t *p = reinterpret_cast<uint64_t *>(
  ...
  return (pvalue >>
          (((index % slots_per_block_) * metadata_->bits_per_slot) % 8)) &
         BITMASK(size_to_get);
}

inline uint64_t Memento::get_slot_payload(uint64_t index) const {
  assert(index < metadata_->xnslots);
  /* Should use __uint128_t to support up to 64-bit remainders, but gcc seems
   * to generate buggy code.  :/  */
  ...
  uint64_t *p = reinterpret_cast<uint64_t *>(
      &get_block(index / slots_per_block_)
           ->slots[((index % slots_per_block_) * metadata_->bits_per_slot +
                    // skip over the fingerprint and memento bits
                    metadata_->fingerprint_bits + metadata_->memento_bits) / 8]);
  // you cannot just do *p to get the value, undefined behavior
  uint64_t pvalue;
  memcpy(&pvalue, p, sizeof(pvalue));
  return (pvalue >>
          (((index % slots_per_block_) * metadata_->bits_per_slot +
            // skip over the fingerprint and memento bits
            metadata_->fingerprint_bits + metadata_->memento_bits) % 8)) &
         BITMASK(size_to_get);
}

inline void Memento::set_slot_payload(uint64_t index, uint64_t payload) {
  assert(index < metadata_->xnslots);
  /* Should use __uint128_t to support up to 64-bit remainders, but gcc seems
   * to generate buggy code.  :/  */
  uint64_t size_to_set = metadata_->payload_bits;
  uint64_t *p = reinterpret_cast<uint64_t *>(
      &get_block(index / slots_per_block_)
           ->slots[((index % slots_per_block_) * metadata_->bits_per_slot +
                    // skip over the fingerprint and memento bits
  ...
  uint64_t t;
  memcpy(&t, p, sizeof(t));
  uint64_t mask = BITMASK(size_to_set);
  uint64_t v = payload;
  int32_t shift = ((index % slots_per_block_) * metadata_->bits_per_slot +
                   // skip over the fingerprint and memento bits
                   metadata_->fingerprint_bits + metadata_->memento_bits) % 8;
  mask <<= shift;
  v <<= shift;
...
inline void Memento::set_slot(uint64_t index, uint64_t value) {
  assert(index < metadata_->xnslots);
  /* Should use __uint128_t to support up to 64-bit remainders, but gcc seems
   * to generate buggy code.  :/  */
  uint64_t size_to_set = metadata_->fingerprint_bits + metadata_->memento_bits;
  uint64_t *p = reinterpret_cast<uint64_t *>(
      &get_block(index / slots_per_block_)
           ->slots[(index % slots_per_block_) * metadata_->bits_per_slot / 8]);
  // This results in undefined behavior:
...
inline uint64_t Memento::block_offset(uint64_t blockidx) const {
  ...
  return run_end(slots_per_block_ * blockidx - 1) -
         slots_per_block_ * blockidx + 1;
}

inline uint64_t Memento::run_end(uint64_t hash_bucket_index) const {
  ...
  uint64_t bucket_intrablock_rank = bitrank(
      get_block(bucket_block_index)->occupieds[0], bucket_intrablock_offset);
  ...
  uint64_t runend_block_index =
      bucket_block_index + bucket_blocks_offset / slots_per_block_;
  ...
  uint64_t runend_block_offset =
      bitselectv(get_block(runend_block_index)->runends[0], runend_ignore_bits,
                 runend_rank);
  if (runend_block_offset == slots_per_block_) {
    if (bucket_blocks_offset == 0 && bucket_intrablock_rank == 0) {
      ...
      do {
        ...
        runend_block_offset =
            bitselectv(get_block(runend_block_index)->runends[0],
  ...
  uint64_t runend_index =
      slots_per_block_ * runend_block_index + runend_block_offset;
...
inline int32_t Memento::offset_lower_bound(uint64_t slot_index) const {
...
inline uint64_t Memento::find_first_empty_slot(uint64_t from) const {
  do {
...
inline uint64_t Memento::get_number_of_consecutive_empty_slots(
    uint64_t first_empty, uint64_t goal_slots) const {
  uint64_t inter_block_offset = first_empty % slots_per_block_;
  uint64_t occupieds =
      METADATA_WORD(occupieds, first_empty) & (~BITMASK(inter_block_offset));
  ...
  while (true) {
    ...
    if (empty_bits < 64 || res >= goal_slots) break;
...
inline void Memento::shift_remainders(const uint64_t start_index,
  ...
  while (last_word != first_word) {
    *REMAINDER_WORD(last_word) =
        shift_into_b(*REMAINDER_WORD(last_word - 1), *REMAINDER_WORD(last_word),
                     0, bend, metadata_->bits_per_slot);
    last_word--;
    bend = 64;
  }
  *REMAINDER_WORD(last_word) = shift_into_b(
      0, *REMAINDER_WORD(last_word), bstart, bend, metadata_->bits_per_slot);
}

inline void Memento::find_next_n_empty_slots(uint64_t from, uint64_t n,
                                             uint64_t *indices) const {
  while (n) {
...
inline uint32_t Memento::find_next_empty_slot_runs_of_size_n(
    uint64_t from, uint64_t n, uint64_t *indices) const {
  uint32_t ind = 0;
  while (n > 0) {
...
inline void Memento::shift_slots(int64_t first, uint64_t last,
                                 uint64_t distance) {
  if (distance == 1 && metadata_ -> payload_bits == 0) {
  ...
  */
  ...
  int64_t x_last_bit =
      last * metadata_->bits_per_slot + metadata_->bits_per_slot - 1;
  ...
  do {
    ...
    int32_t move_bits =
        8 * mn +
        (x_last_bit % 8 < y_last_bit % 8 ? x_last_bit % 8 : y_last_bit % 8) + 1;
    ...
    payload =
        (w_j & (BITMASK(y_prefix) | (BITMASK(y_extra) << (64 - y_extra)))) |
...
inline void Memento::shift_runends(int64_t first, uint64_t last,
                                   uint64_t distance) {
  ...
  if (last_word != first_word) {
    ...
    const uint64_t first_runends_replacement =
        METADATA_WORD(runends, first) & (~BITMASK(bstart));
    METADATA_WORD(runends, 64 * last_word) =
        shift_into_b(last_word == first_word + 1
    ...
    last_word--;
    while (last_word != first_word) {
      METADATA_WORD(runends, 64 * last_word) = shift_into_b(
  ...
  METADATA_WORD(runends, 64 * last_word) = shift_into_b(
      0LL, METADATA_WORD(runends, 64 * last_word), bstart, bend, distance);
}

inline int32_t
Memento::remove_replace_slots_and_shift_remainders_and_runends_and_offsets(
    bool operation, uint64_t bucket_index, uint64_t overwrite_index,
    const uint64_t *remainders, uint64_t total_remainders,
    uint64_t old_length) {
  uint64_t i;
  ...
  if (is_runend(overwrite_index + old_length - 1)) {
    if (total_remainders > 0) {
      ...
      // If we're deleting this entry entirely, but it is not the first entry
      // in this run, then set the preceding entry to be the runend
      METADATA_WORD(runends, overwrite_index - 1) |=
  ...
  // Reset the occupied bit of the hash bucket index if the hash is the only
  // item in the run and is removed completely.
  ...
  while (1 && old_length > total_remainders) {
    ...
    int32_t last_occupieds_bit =
        highbit_position(get_block(original_block)->occupieds[0]);
    // There is nothing in the block
    if (last_occupieds_bit == 64) {
      if (get_block(original_block + 1)->offset == 0) break;
      get_block(original_block + 1)->offset = 0;
    } else {
      uint64_t last_occupieds_hash_index =
          slots_per_block_ * original_block + last_occupieds_bit;
      ...
      if (runend_index / slots_per_block_ == original_block) {
        ...
        if (get_block(original_block + 1)->offset ==
            (runend_index % slots_per_block_) + 1)
          break;
        get_block(original_block + 1)->offset =
            (runend_index % slots_per_block_) + 1;
      } else {
        // If the last run spans across multiple blocks
        for (i = original_block + 1; i < runend_index / slots_per_block_ - 1;
             i++)
          get_block(i)->offset = slots_per_block_;
        if (get_block(runend_index / slots_per_block_)->offset ==
            (runend_index % slots_per_block_) + 1)
          break;
        get_block(runend_index / slots_per_block_)->offset =
            (runend_index % slots_per_block_) + 1;
      }
    }
    original_block++;
  ...
  if (!total_remainders) modify_metadata(&metadata_->ndistinct_elts, -1);

  return ret_current_distance;
}

inline int32_t Memento::make_empty_slot_for_memento_list(uint64_t bucket_index,
                                                         uint64_t pos) {
  const uint64_t next_empty = find_first_empty_slot(pos);
  if (next_empty >= metadata_->xnslots) {  // Check that the new data fits
    return err_no_space;
  }
  if (pos < next_empty) shift_slots(pos, next_empty - 1, 1);
...
inline int32_t Memento::make_n_empty_slots_for_memento_list(
    uint64_t bucket_index, uint64_t pos, uint32_t n) {
  uint64_t empty_runs[2 * n];
  uint64_t empty_runs_ind =
      find_next_empty_slot_runs_of_size_n(pos, n, empty_runs);
  ...
  if (pos < empty_runs[0]) shift_slots(pos, empty_runs[0] - 1, n);
  shift_runends(pos - 1, empty_runs[0] - 1, n);
  // Update offsets
  uint64_t npreceding_empties = 0;
  uint32_t empty_iter = 0;
  uint32_t last_block_to_update_offset =
      (empty_runs[empty_runs_ind - 2] + empty_runs[empty_runs_ind - 1] - 1) /
      slots_per_block_;
  for (uint64_t i = bucket_index / slots_per_block_ + 1;
    ...
    while (npreceding_empties < n) {
      ...
      uint64_t empty_run_end =
          empty_runs[empty_iter] + empty_runs[empty_iter + 1];
      if (r <= empty_run_start) break;
      if (l < empty_run_start) l = empty_run_start;
      if (r > empty_run_end) {
        r = empty_run_end;
    ...
    if (get_block(i)->offset + n - npreceding_empties <
        BITMASK(8 * sizeof(blocks_[0].offset)))
      get_block(i)->offset += n - npreceding_empties;
    else
      get_block(i)->offset =
          static_cast<uint8_t>(BITMASK(8 * sizeof(blocks_[0].offset)));
  }
  ...
  if (memento_cnt > 2) {
    ...
    if (list_len >= max_memento_value) {
      ...
      for (uint32_t i = 0; i < frag_cnt - 1; i++) {
        APPEND_WRITE_PAYLOAD_WORD(payload, current_full_prefix,
                                  max_memento_value, memento_bits, dest_bit_pos,
                                  dest_block_ind);
      }
    ...
    if (written_bits < 0) {
      APPEND_WRITE_PAYLOAD_WORD(payload, current_full_prefix, 0LL,
                                -written_bits, dest_bit_pos, dest_block_ind);
    }
  }
  if (current_full_prefix) {
    FLUSH_PAYLOAD_WORD(payload, current_full_prefix, dest_bit_pos,
...
inline int32_t Memento::remove_mementos_from_prefix_set(
  ...
  *old_slot_count = 2;
  ...
  uint64_t data_bit_pos =
      ((pos + 2) % slots_per_block_) * metadata_->bits_per_slot;
  ...
  if (res_cnt != old_memento_cnt) {
    ...
    if (old_memento_cnt > 2) {
      int32_t old_list_bits =
          (old_memento_cnt - 1 + 2 * old_unary_cnt) * memento_bits;
      while (old_list_bits > 0) {
        old_list_bits -= metadata_->bits_per_slot;
        (*old_slot_count)++;
      }
    }
    *new_slot_count =
...
inline int32_t Memento::add_memento_to_sorted_list(const uint64_t bucket_index,
  ...
  // size 2 => min than max
  // size > 2 => max, min, list size, rest of the mementos
  const bool singleton_prefix_set = (is_runend(pos) || f1 <= f2);
  if (singleton_prefix_set) {
    // this case is list encoding because we cannot distinguish between size 2 and larger
    if (new_memento == m1) {
      int32_t err =
    ...
    if (metadata_->bits_per_slot < 2 * metadata_->memento_bits + metadata_->payload_bits) {
      ...
      if (new_memento < m1) {
        if (metadata_->payload_bits > 0) {
          ...
          set_slot_payload(pos + 1, new_payload);
          // set the payload after the memento
          ...
          INIT_PAYLOAD_WORD(payload, current_full_prefix, dest_bit_pos,
                            dest_block_ind);
          uint64_t value = (m1 << memento_bits) | 1ULL;
          APPEND_WRITE_PAYLOAD_WORD(payload, current_full_prefix, value,
        ...
        if (metadata_ -> payload_bits > 0) {
          ...
          INIT_PAYLOAD_WORD(payload, current_full_prefix, dest_bit_pos,
                            dest_block_ind);
          uint64_t value = (m2 << memento_bits) | 1ULL;
          APPEND_WRITE_PAYLOAD_WORD(payload, current_full_prefix, value,
                                    metadata_->memento_bits + metadata_->memento_bits, dest_bit_pos,
                                    dest_block_ind);
          APPEND_WRITE_PAYLOAD_WORD(payload, current_full_prefix, m2_payload,
...
inline uint64_t Memento::number_of_slots_used_for_memento_list(
    uint64_t pos) const {
...
inline int64_t Memento::next_matching_fingerprint_in_run(
    uint64_t pos, const uint64_t fingerprint) const {
  ...
  while (true) {
    ...
    if (fingerprint < current_fingerprint) return -1;
...
inline uint64_t Memento::lower_bound_fingerprint_in_run(
    uint64_t pos, uint64_t fingerprint) const {
  uint64_t current_fingerprint, current_memento;
  uint64_t next_fingerprint, next_memento;
  do {
    current_fingerprint = get_fingerprint(pos);
...
inline uint64_t Memento::upper_bound_fingerprint_in_run(
    uint64_t pos, uint64_t fingerprint) const {
  uint64_t current_fingerprint, current_memento;
  do {
    current_fingerprint = get_fingerprint(pos);
...
inline int32_t Memento::insert_mementos(const __uint128_t hash,
                                        const uint64_t mementos[],
                                        const uint64_t memento_count,
                                        const uint32_t actual_fingerprint_size,
                                        uint8_t runtime_lock) {
  ...
  const uint64_t hash_bucket_index =
      ((hash & BITMASK(orig_quotient_size))
       << (bucket_index_hash_size - orig_quotient_size)) |
      ((hash >> orig_quotient_size) &
  ...
  if (hash_fingerprint && memento_count > 2) {
    ...
    if (memento_count - 2 >= max_memento_value) {
      ...
      for (uint32_t tmp_cnt = val; tmp_cnt < memento_count - 2;
           tmp_cnt += val) {
    ...
    while (total_new_bits > 0) {
      ...
      new_slot_count++;
    }  // Result of new_slot_count same as using normal division:
       // `total_new_bits / metadata->bits_per_slot` Hopefully this is a bit
       // faster to calculate.
  } else if (hash_fingerprint != 0 &&
             (memento_count == 2 && mementos[0] == mementos[1])) {
    new_slot_count = 3;
  }
  ...
  uint64_t empty_runs_ind = find_next_empty_slot_runs_of_size_n(
      hash_bucket_index, new_slot_count, empty_runs);
  if (empty_runs[empty_runs_ind - 2] + empty_runs[empty_runs_ind - 1] - 1 >=
      metadata_->xnslots) {
    // Check that the new data fits
    if (GET_NO_LOCK(runtime_lock) != flag_no_lock) {
  ...
  for (uint64_t i = hash_bucket_index / slots_per_block_ + 1;
    ...
    if (get_block(i)->offset + new_slot_count - npreceding_empties <
        BITMASK(8 * sizeof(blocks_[0].offset)))
  ...
  uint64_t runstart_index =
      hash_bucket_index == 0 ? 0 : run_end(hash_bucket_index - 1) + 1;
  uint64_t insert_index;
  if (is_occupied(hash_bucket_index)) {
    insert_index =
        upper_bound_fingerprint_in_run(runstart_index, hash_fingerprint);
    ...
    METADATA_WORD(runends, runend_index) &=
        ~(1ULL << ((runend_index % slots_per_block_) % 64));
    METADATA_WORD(runends, runend_index + new_slot_count) |=
        1ULL << (((runend_index + new_slot_count) % slots_per_block_) % 64);
  } else {
    ...
    METADATA_WORD(runends, insert_index + new_slot_count - 1) |=
        1ULL << (((insert_index + new_slot_count - 1) % slots_per_block_) % 64);
    METADATA_WORD(occupieds, hash_bucket_index) |=
...
inline Memento::Memento(uint64_t nslots, uint64_t key_bits,
                        uint64_t memento_bits, hashmode hash_mode,
                        uint32_t seed, const uint64_t orig_quotient_bit_cnt,
                        uint64_t payload_bits) {
  ...
  metadata_->original_quotient_bits =
      (orig_quotient_bit_cnt ? orig_quotient_bit_cnt
  ...
  metadata_->nblocks =
      (metadata_->xnslots + slots_per_block_ - 1) / slots_per_block_;
  metadata_->nelts = 0;
  ...
  runtimedata_->locks = reinterpret_cast<volatile int32_t *>(
      new volatile int32_t[runtimedata_->num_locks]);
#ifdef LOG_WAIT_TIME
  runtimedata->wait_times = reinterpret_cast<wait_time_data *>(
      new wait_time_data[runtimedata->num_locks + 1]);
#endif
}
...
inline Memento::Memento(const Memento &other) {
  ...
  runtimedata_->locks = reinterpret_cast<volatile int32_t *>(
      new volatile int32_t[other.runtimedata_->num_locks]);
  ...
  memcpy(metadata->wait_times, memento.metadata->wait_times,
         sizeof(wait_time_data) * (runtimedata->num_locks + 1));
#endif
}

inline Memento::Memento(Memento &&other) noexcept {
...
inline Memento &Memento::operator=(const Memento &other) {
  delete[] runtimedata_->locks;
  delete[] runtimedata_;
#ifdef LOG_WAIT_TIME
  delete[] runtimedata->wait_times;
...
inline Memento &Memento::operator=(Memento &&other) noexcept {
  metadata_ = other.metadata_;
  blocks_ = other.blocks_;
  runtimedata_ = other.runtimedata_;
  other.metadata_ = nullptr;
...
inline int64_t Memento::resize(uint64_t nslots) {
  Memento new_memento(nslots, metadata_->key_bits, metadata_->memento_bits,
...
inline int32_t Memento::insert_mementos(uint64_t key, uint64_t mementos[],
  ...
  const uint64_t orig_nslots =
      metadata_->nslots >> (metadata_->key_bits - metadata_->fingerprint_bits -
                            metadata_->original_quotient_bits);
  uint64_t fast_reduced_part =
      fast_reduce(((key & BITMASK(metadata_->original_quotient_bits))
                   << (32 - metadata_->original_quotient_bits)),
                  orig_nslots);
  ...
  if (ret > (int32_t)distance_from_home_slot_cutoff_ &&
      metadata_->auto_resize) {
    resize(metadata_->nslots * 2);
  }
  return ret;
}

inline int64_t Memento::insert(uint64_t key, uint64_t memento, uint8_t flags, uint64_t payload) {
  // We fill up the CQF up to 95% load factor.
  // This is a very conservative check.
  if (metadata_->noccupied_slots >= metadata_->nslots * 0.95 ||
  ...
  const uint64_t orig_nslots =
      metadata_->nslots >> (metadata_->key_bits - metadata_->fingerprint_bits -
                            metadata_->original_quotient_bits);
  const uint64_t fast_reduced_part =
      fast_reduce(((key & BITMASK(metadata_->original_quotient_bits))
                   << (32 - metadata_->original_quotient_bits)),
                  orig_nslots);
  key &= ~BITMASK(metadata_->original_quotient_bits);
  ...
  const uint32_t orig_quotient_size = metadata_->original_quotient_bits;
  const uint64_t hash_bucket_index =
      (fast_reduced_part << (bucket_index_hash_size - orig_quotient_size)) |
      ((hash >> orig_quotient_size) &
       BITMASK(bucket_index_hash_size - orig_quotient_size));
  ...
  if (is_occupied(hash_bucket_index)) {
    ...
    fingerprint_pos =
        next_matching_fingerprint_in_run(fingerprint_pos, hash_fingerprint);
    ...
    if (add_to_sorted_list) {
      ...
      for (uint32_t i = hash_bucket_index / slots_per_block_ + 1;
        ...
        if (get_block(i)->offset + 1ULL <=
            BITMASK(8 * sizeof(blocks_[0].offset)))
          get_block(i)->offset++;
      }
      set_slot(insert_index,
               (hash_fingerprint << metadata_->memento_bits) | memento);
      if (metadata_->payload_bits > 0) {
        set_slot_payload(insert_index, payload);
      }
      METADATA_WORD(runends, runend_index) &=
          ~(1ULL << ((runend_index % slots_per_block_) % 64));
      METADATA_WORD(runends, runend_index + 1) |=
          1ULL << (((runend_index + 1) % slots_per_block_) % 64);
      modify_metadata(&metadata_->ndistinct_elts, 1);
      modify_metadata(&metadata_->noccupied_slots, 1);
      res = insert_index - hash_bucket_index;
    }
    ...
    METADATA_WORD(runends, insert_index) |=
        1ULL << ((insert_index % slots_per_block_) % 64);
    METADATA_WORD(occupieds, hash_bucket_index) |=
        1ULL << ((hash_bucket_index % slots_per_block_) % 64);
    modify_metadata(&metadata_->ndistinct_elts, 1);
    modify_metadata(&metadata_->noccupied_slots, 1);
...
inline void Memento::bulk_load(uint64_t *sorted_hashes, uint64_t n,
                               uint8_t flags) {
  (void)(flags);  // Unused
  ...
  for (uint64_t i = 1; i < n; i++) {
    ...
    if (new_prefix == prefix)
      ...
      const uint32_t slots_written =
          write_prefix_set(current_pos, prefix & fingerprint_mask, memento_list,
                           prefix_set_size);
      ...
      if (current_run != next_run) {
        ...
        for (uint64_t block_ind = current_run / slots_per_block_ + 1;
          ...
          const uint32_t cnt =
              current_pos - (block_ind * slots_per_block_ < old_pos
                                 ? old_pos
                                 : block_ind * slots_per_block_);
          if (get_block(block_ind)->offset + cnt <
              BITMASK(8 * sizeof(blocks_[0].offset)))
            get_block(block_ind)->offset += cnt;
          else
            get_block(block_ind)->offset =
                static_cast<uint8_t>(BITMASK(8 * sizeof(blocks_[0].offset)));
        }
  ...
  const uint32_t slots_written = write_prefix_set(
      current_pos, prefix & fingerprint_mask, memento_list, prefix_set_size);
  current_pos += slots_written;
  total_slots_written += slots_written;
  METADATA_WORD(occupieds, current_run) |=
      (1ULL << ((current_run % slots_per_block_) % 64));
  ...
  for (uint64_t block_ind = current_run / slots_per_block_ + 1;
       block_ind <= (current_pos - 1) / slots_per_block_; block_ind++) {
    const uint32_t cnt = current_pos - (block_ind * slots_per_block_ < old_pos
                                            ? old_pos
                                            : block_ind * slots_per_block_);
    if (get_block(block_ind)->offset + cnt <
        BITMASK(8 * sizeof(blocks_[0].offset)))
...
inline int32_t Memento::delete_single(uint64_t key, uint64_t memento,
                                      uint8_t flags) {
  if (GET_KEY_HASH(flags) != flag_key_is_hash) {
    if (metadata_->hash_mode == hashmode::Default)
      key = MurmurHash64A(&key, sizeof(key), metadata_->seed);
  ...
  const uint64_t orig_nslots =
      metadata_->nslots >> (metadata_->key_bits - metadata_->fingerprint_bits -
                            metadata_->original_quotient_bits);
  const uint64_t fast_reduced_part = fast_reduce(
      ((key & BITMASK(orig_quotient_size)) << (32 - orig_quotient_size)),
      orig_nslots);
  key &= ~(BITMASK(metadata_->original_quotient_bits));
  key |= fast_reduced_part;
  uint64_t hash = key;
    ...
    if (handled) {
      if (new_slot_count < old_slot_count) {
        ...
        remove_replace_slots_and_shift_remainders_and_runends_and_offsets(
            ...
            nullptr, 0, old_slot_count - new_slot_count);
      }
      break;
    }
  }
...
inline uint64_t Memento::lower_bound_mementos_for_fingerprint(
    uint64_t pos, uint64_t target_memento) const {
  ...
  if (current_memento < next_memento) {
    ...
    // Mementos encoded as a sorted list
    if (target_memento <= next_memento) return next_memento;
    uint64_t max_memento = current_memento;
    if (max_memento <= target_memento) return max_memento;
    ...
    uint32_t current_full_bits =
        metadata_->bits_per_slot - metadata_->memento_bits;
...
inline int32_t Memento::point_query(uint64_t key, uint64_t memento,
  ...
  const uint64_t orig_nslots =
      metadata_->nslots >> (metadata_->key_bits - metadata_->fingerprint_bits -
                            metadata_->original_quotient_bits);
  const uint64_t fast_reduced_part =
      fast_reduce(((hash & BITMASK(metadata_->original_quotient_bits))
                   << (32 - metadata_->original_quotient_bits)),
                  orig_nslots);
  const uint64_t hash_bucket_index =
      (fast_reduced_part << (bucket_index_hash_size - orig_quotient_size)) |
  ...
  if (!is_occupied(hash_bucket_index)) return false;
  ...
  while (true) {
    ...
    const uint64_t current_fingerprint = get_fingerprint(fingerprint_pos);
    const uint64_t next_fingerprint = get_fingerprint(fingerprint_pos + 1);
    const int positive_res =
        (highbit_position(current_fingerprint) == metadata_->fingerprint_bits
             ? 1
             : 2);
    if (!is_runend(fingerprint_pos) && current_fingerprint > next_fingerprint) {
      if (lower_bound_mementos_for_fingerprint(fingerprint_pos, memento) ==
          memento) {
        if (payload != nullptr && metadata_->payload_bits > 0) {
          *payload = get_slot_payload(fingerprint_pos);
        }
        // set also the payload if exists
        return positive_res;
      }
...
inline int32_t Memento::range_query(uint64_t l_key, uint64_t l_memento,
  ...
  const uint64_t l_fast_reduced_part =
      fast_reduce(((l_hash & BITMASK(metadata_->original_quotient_bits))
                   << (32 - metadata_->original_quotient_bits)),
                  orig_nslots);
  const uint64_t l_hash_bucket_index =
      (l_fast_reduced_part << (bucket_index_hash_size - orig_quotient_size)) |
      ((l_hash >> orig_quotient_size) &
       BITMASK(bucket_index_hash_size - orig_quotient_size));
  const uint64_t l_hash_fingerprint =
      (l_hash >> bucket_index_hash_size) & BITMASK(metadata_->fingerprint_bits);

  const uint64_t r_hash = r_key;
  const uint64_t r_fast_reduced_part =
      fast_reduce(((r_hash & BITMASK(metadata_->original_quotient_bits))
                   << (32 - metadata_->original_quotient_bits)),
                  orig_nslots);
  const uint64_t r_hash_bucket_index =
      (r_fast_reduced_part << (bucket_index_hash_size - orig_quotient_size)) |
      ((r_hash >> orig_quotient_size) &
       BITMASK(bucket_index_hash_size - orig_quotient_size));
  const uint64_t r_hash_fingerprint =
      (r_hash >> bucket_index_hash_size) & BITMASK(metadata_->fingerprint_bits);
  ...
  if (l_hash == r_hash) {  // Range contained in a single prefix.
    ...
    uint64_t runstart_index =
        l_hash_bucket_index == 0 ? 0 : run_end(l_hash_bucket_index - 1) + 1;
    ...
    while (true) {
      fingerprint_pos =
          next_matching_fingerprint_in_run(fingerprint_pos, l_hash_fingerprint);
      if (fingerprint_pos < 0) {
        // Matching fingerprints exhausted
        break;
      }
      ...
      if (!is_runend(fingerprint_pos) &&
        ...
        candidate_memento =
            lower_bound_mementos_for_fingerprint(fingerprint_pos, l_memento);
    ...
    if (!is_occupied(r_hash_bucket_index))
      ...
      r_runstart_index =
          r_hash_bucket_index == 0 ? 0 : run_end(r_hash_bucket_index - 1) + 1;
    ...
    if (l_runstart_index < metadata_->xnslots) {
      ...
      while (true) {
        fingerprint_pos = next_matching_fingerprint_in_run(fingerprint_pos,
                                                           l_hash_fingerprint);
        if (fingerprint_pos < 0) {
          // Matching fingerprints exhausted
          break;
        ...
        const uint64_t current_fingerprint = get_fingerprint(fingerprint_pos);
        const uint64_t next_fingerprint = get_fingerprint(fingerprint_pos + 1);
        const int positive_res = (highbit_position(current_fingerprint) ==
        ...
        if (!is_runend(fingerprint_pos) &&
          ...
          if (has_sorted_list && l_memento <= m1) return positive_res;
          if (!has_sorted_list && l_memento <= m2) return positive_res;
          ...
          if (l_memento <= get_memento(fingerprint_pos)) return positive_res;
          fingerprint_pos++;
        }
    ...
    for (uint64_t mid_key = orig_l_key + 1; mid_key < orig_r_key; mid_key++) {
      ...
      if (GET_KEY_HASH(flags) != flag_key_is_hash) {
        if (metadata_->hash_mode == hashmode::Default) {
          mid_hash =
              MurmurHash64A(&mid_hash, sizeof(mid_hash), metadata_->seed);
        } else if (metadata_->hash_mode == hashmode::Invertible) {
          mid_hash = hash_64(mid_hash, BITMASK(63));
        }
      }
      const uint64_t mid_fast_reduced_part =
          fast_reduce(((mid_hash & BITMASK(metadata_->original_quotient_bits))
                       << (32 - metadata_->original_quotient_bits)),
                      orig_nslots);
      const uint64_t mid_hash_bucket_index =
          (mid_fast_reduced_part
           << (bucket_index_hash_size - orig_quotient_size)) |
          ((mid_hash >> orig_quotient_size) &
           BITMASK(bucket_index_hash_size - orig_quotient_size));
      const uint64_t mid_hash_fingerprint =
          (mid_hash >> bucket_index_hash_size) &
          BITMASK(metadata_->fingerprint_bits);

      if (!is_occupied(mid_hash_bucket_index)) continue;
      uint64_t mid_runstart_index =
          mid_hash_bucket_index == 0 ? 0
      ...
      if (mid_runstart_index < metadata_->xnslots) {
        ...
        int64_t fingerprint_pos = next_matching_fingerprint_in_run(
            mid_runstart_index, mid_hash_fingerprint);
        if (fingerprint_pos >= 0) {
    ...
    if (r_runstart_index < metadata_->xnslots) {
      ...
      while (true) {
        ...
        if (!is_runend(fingerprint_pos) &&
          ...
          if (has_sorted_list && m2 <= r_memento) return positive_res;
          if (!has_sorted_list && m1 <= r_memento) return positive_res;
          ...
          if (get_memento(fingerprint_pos) <= r_memento) return positive_res;
          fingerprint_pos++;
        }
...
inline Memento::iterator Memento::end() const { return iterator(*this); }
...
inline Memento::iterator::iterator(const Memento &filter, const uint64_t l_key,
...
inline void Memento::iterator::fetch_matching_prefix_mementos() {
  ...
  uint64_t cur_prefix_hash =
      MurmurHash64A(&cur_prefix_, sizeof(cur_prefix_), filter_.get_hash_seed());
  ...
  const uint64_t orig_nslots =
      filter_.metadata_->nslots >>
      (filter_.metadata_->key_bits - filter_.metadata_->fingerprint_bits -
       filter_.metadata_->original_quotient_bits);
  const uint64_t fast_reduced_part =
      fast_reduce(((cur_prefix_hash & BITMASK(orig_quotient_size))
                   << (32 - orig_quotient_size)),
                  orig_nslots);
  const uint64_t hash_bucket_index =
      (fast_reduced_part << (bucket_index_hash_size - orig_quotient_size)) |
      ((cur_prefix_hash >> orig_quotient_size) &
       BITMASK(bucket_index_hash_size - orig_quotient_size));
  const uint64_t hash_fingerprint =
...
inline Memento::iterator &Memento::iterator::operator++() {
  if (cur_ind_ < mementos_.size() - 1) {
    ...
    const uint64_t cur_key =
        (cur_prefix_ << filter_.get_num_memento_bits()) | mementos_[cur_ind_];
    if (cur_key > r_key_) cur_prefix_ = std::numeric_limits<uint64_t>::max();
  } else {
...
inline Memento::iterator::iterator(const iterator &other)
...
inline bool Memento::iterator::operator==(const iterator &rhs) const {
  if (&filter_ != &rhs.filter_) return false;
  if (cur_prefix_ == std::numeric_limits<uint64_t>::max() &&
      rhs.cur_prefix_ == std::numeric_limits<uint64_t>::max())
    return true;
...
inline Memento::hash_iterator Memento::hash_begin(uint64_t position) const {
  ...
  if (!is_occupied(position)) {
    ...
    uint64_t idx = lowbit_position(get_block(block_index)->occupieds[0] &
                                   (~BITMASK(position % slots_per_block_)));
    if (idx == 64) {
...
inline Memento::hash_iterator Memento::hash_begin(uint64_t key,
                                                  uint8_t flags) const {
  hash_iterator it(*this);
  ...
  const uint64_t orig_nslots =
      metadata_->nslots >> (metadata_->key_bits - metadata_->fingerprint_bits -
                            metadata_->original_quotient_bits);
  const uint64_t fast_reduced_part = fast_reduce(
      ((hash & BITMASK(orig_quotient_size)) << (32 - orig_quotient_size)),
      orig_nslots);
  const uint64_t hash_bucket_index =
      (fast_reduced_part << (bucket_index_hash_size - orig_quotient_size)) |
      ((hash >> orig_quotient_size) &
  ...
  if (is_occupied(hash_bucket_index)) {
    uint64_t runstart_index =
        hash_bucket_index == 0 ? 0 : run_end(hash_bucket_index - 1) + 1;
    if (runstart_index < hash_bucket_index) runstart_index = hash_bucket_index;
    int64_t fingerprint_pos =
        next_matching_fingerprint_in_run(runstart_index, hash_fingerprint);
    if (fingerprint_pos < 0)
      fingerprint_pos =
          lower_bound_fingerprint_in_run(runstart_index, hash_fingerprint);
  ...
  if (!is_occupied(hash_bucket_index) || !target_found) {
    ...
    uint64_t block_index = position / slots_per_block_;
    uint64_t idx =
        lowbit_position(get_block(block_index)->occupieds[0] &
                        (~BITMASK((position % slots_per_block_) + 1)));
    if (idx == 64) {
      while (idx == 64 && block_index < metadata_->nblocks) {
        block_index++;
        idx = lowbit_position(get_block(block_index)->occupieds[0]);
...
inline int32_t Memento::hash_iterator::get(uint64_t &key,
  ...
  if (!filter_.is_runend(current_) && f1 > f2) {
    ...
    if (m1 < m2) {
      ...
      uint64_t filled_bits = 0;
      uint64_t data_bit_pos =
          (pos % slots_per_block_) * filter_.metadata_->bits_per_slot;
  ...
  const uint32_t bucket_index_hash_size = filter_.get_bucket_index_hash_size();
  const uint32_t original_quotient_bits =
      filter_.metadata_->original_quotient_bits;
  const uint64_t original_bucket_index =
      run_ >> (bucket_index_hash_size - original_quotient_bits);
  const uint64_t bucket_extension =
      ((run_ & BITMASK(bucket_index_hash_size - original_quotient_bits))
       << original_quotient_bits);
  key =
      original_bucket_index | (f1 << bucket_index_hash_size) | bucket_extension;
  return res;
}

inline Memento::hash_iterator::hash_iterator(hash_iterator &&other) noexcept
    : filter_{other.filter_}, run_{other.run_}, current_{other.current_} {
#ifdef LOG_CLUSTER_LENGTH
...
inline Memento::hash_iterator &Memento::hash_iterator::operator=(
    const hash_iterator &other) {
  assert(&filter_ == &other.filter_);
  run_ = other.run_;
  ...
  if (c_info == nullptr && other.c_info != nullptr) {
    ...
    memcpy(c_info, other.c_info,
           sizeof(cluster_data) * (filter.metadata->nslots / 32));
...
inline Memento::hash_iterator &Memento::hash_iterator::operator=(
  ...
  cur_start_index = other.cur_start_index;
  cur_length = other.cur_length;
  if (c_info != nullptr) delete c_info;
  c_info = other.c_info;
  other.c_info = nullptr;
#endif
  return *this;
...
inline Memento::hash_iterator &Memento::hash_iterator::operator++() {
  if (*this == filter_.hash_end())
    ...
    if (!filter_.is_runend(current_)) {
      if (filter_.get_fingerprint(current_) >
          filter_.get_fingerprint(current_ + 1)) {
        uint64_t current_memento = filter_.get_memento(current_);
        uint64_t next_memento = filter_.get_memento(current_ + 1);
        if (current_memento < next_memento)
          current_++;
        else  // Mementos encoded as a sorted list
          current_ +=
              filter_.number_of_slots_used_for_memento_list(current_ + 2) + 1;
      }
    }
    ...
    if (!filter_.is_runend(current_)) {
      ...
      uint64_t rank = bitrank(filter_.get_block(block_index)->occupieds[0],
                              run_ % slots_per_block_);
      uint64_t next_run =
          bitselect(filter_.get_block(block_index)->occupieds[0], rank);
      if (next_run == 64) {
        rank = 0;
      ...
      current_++;
      if (current_ < run_) current_ = run_;
#ifdef LOG_CLUSTER_LENGTH
...
inline Memento::hash_iterator Memento::hash_iterator::operator++(int) {
  auto old = *this;
  operator++();
  return old;
}
...
inline bool Memento::hash_iterator::is_at_runend() const {
  if (filter_.is_runend(current_)) return true;
  uint64_t current_memento = filter_.get_memento(current_);
  uint64_t next_memento = filter_.get_memento(current_ + 1);
  if (current_memento < next_memento) return false;
  return filter_.is_runend(
      current_ + filter_.number_of_slots_used_for_memento_list(current_ + 2) +
      1);
}

inline bool Memento::hash_iterator::operator==(
    const Memento::hash_iterator &rhs) const {
  if (&filter_ != &rhs.filter_) return false;
  const int64_t lhs_current =
      current_ > filter_.metadata_->xnslots ? -1 : current_;
  const int64_t rhs_current =
      rhs.current_ > filter_.metadata_->xnslots ? -1 : rhs.current_;
...
inline void Memento::debug_dump_metadata() const {
